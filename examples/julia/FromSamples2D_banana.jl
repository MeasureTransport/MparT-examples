...
scatter(x[1,:],x[2,:], markercolor=(:blue,0.1), label="Target samples")
axislegend()
fig1
end
...
function obj(coeffs,p)
    ...
    rho_of_map_of_x = logpdf(rho1, map_of_x.T)
...
begin
total_order2 = 2
fixed_mset2 = MParT.FixedMultiIndexSet(2,total_order2)
end
...
begin
transport_map = MParT.TriangularMap((S1,S2))
...
prob_s1 = OptimizationProblem(fcn, u0, p, gtol=1e-4)
res_s1 = solve(prob, BFGS())
end
...
prob_s2 = OptimizationProblem(fcn, u0, p, gtol=1e-4)
res_s2 = solve(prob, BFGS())
end
...
scatter!(ax4, x1_t,x1_t)
lines!(  ax4, x1_t,vec(Evaluate(S1, xx1'[[1],:])),ls=:dash,label="map approximation")
scatter!(ax4, x1_t,vec(Evaluate(S1, xx1'[[1],:])))
axislegend()
fig4
end
...
contour!(ax5, xx[1,:], xx[2,:], map_eval_true)
contour!(ax5, xx[1,:], xx[2,:], vec(map_eval_approx),ls=:dash)
fig5
end
