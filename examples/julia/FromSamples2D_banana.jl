...
# ╔═╡ 76f7fd4e-5531-4bab-90e7-994a6957ff84
using MParT, Distributions, LinearAlgebra, Statistics, Optimization, OptimizationOptimJL, GLMakie, CxxWrap, Format
...
begin
	...
	test_x = collect(hcat(test_x1,test_x2)')
end
...
begin
	...
	scatter!(ax1, x[1,:],x[2,:], color=(:blue,0.1), label="Target samples")
	axislegend()
	fig1
end
...
function obj(coeffs,p)
	...
	eval_x = x[1:inputDim(tri_map), :]
    map_of_x = Evaluate(tri_map, eval_x)
    rho_of_map_of_x = logpdf(rho1, map_of_x)
...
function grad_obj!(du, coeffs, p)
    ...
    grad_rho_of_map_of_x = -CoeffGrad(tri_map, eval_x, map_of_x)

    # Get the gradient of the log determinant with respect to the map coefficients
    grad_log_det = LogDeterminantCoeffGrad(tri_map, eval_x)

    du .= -sum(grad_rho_of_map_of_x + grad_log_det, dims=2)/num_points
end
...
begin
	multis1 = collect(reshape(0:1,2,1))
...
begin
	transport_map = TriangularMap(CxxWrap.StdVector([S1,S2]))
	SetCoeffs(transport_map, vcat(CoeffMap(S1),CoeffMap(S2)))
end
...
# ╔═╡ 2e090778-af3f-403e-8ab0-ae2b967412a7
begin # For plotting and computing reference density
	ref_distribution = MvNormal(I(2))  #standard normal
	grid_1d = range(-5,5,length=100)
	grid = reduce(hcat, [g1, g2] for g1 in grid_1d, g2 in grid_1d)
	ref_pdf_at_grid = pdf(ref_distribution, grid)
end
...
begin
	...
	fig2 = Figure()
	ax2 = Axis(fig2[1,1], title="Before optimization", xlabel=L"r_1", ylabel=L"r_2")
	contour!(ax2, grid[1,:], grid[2,:], ref_pdf_at_grid)
	scatter!(ax2, r_test_before_opt[1,:],r_test_before_opt[2,:], color=(:blue,0.1), label=L"Pushed target samples through $S$")
	axislegend()
	display(fig2)
	
	comp1_objective_val = obj(CoeffMap(S1), (S1, test_x[1:1,:]))
	comp2_objective_val = obj(CoeffMap(S2), (S2, test_x))

	# Print initial coeffs and objective
	println("==================")
	println("Starting coeffs component 1:")
	println(CoeffMap(S1))
	printfmt("Objective value for component 1: {:.2e}\n", comp1_objective_val)
	println("==================")
	println("Starting coeffs component 2:")
	println(CoeffMap(S2))
	printfmt("Objective value for component 2: {:.2e}\n", comp2_objective_val)
	println("==================")
end
...
# ╔═╡ 840bd43e-8430-48cd-9f61-c273caf5837b
begin
	# Optimize
	u0_s1 = CoeffMap(S1)
	p_s1 = (S1, x)
	fcn_s1 = OptimizationFunction(obj, grad=grad_obj!)
	prob_s1 = OptimizationProblem(fcn_s1, u0_s1, p_s1, g_tol=1e-4)
	res_s1 = solve(prob_s1, BFGS())
end
...
# ╔═╡ b4a318d9-b58d-4453-b43a-0f2649512648
begin
	# Optimize
	u0_s2 = CoeffMap(S2)
	p_s2 = (S2, x)
	fcn_s2 = OptimizationFunction(obj, grad=grad_obj!)
	prob_s2 = OptimizationProblem(fcn_s2, u0_s2, p_s2, g_tol=1e-4)
	res_s2 = solve(prob_s2, BFGS())
end

# ╔═╡ 926e427a-016e-4e55-95b3-afa945dbce29
res_s1.retcode, res_s2.retcode
...
begin
	# Before optimization plot
	fig3 = Figure()
	ax3 = Axis(fig3[1,1], title="After optimization", xlabel=L"r_1", ylabel=L"r_2")
	contour!(ax3, grid[1,:], grid[2,:], ref_pdf_at_grid, label="Reference PDF")
	scatter!(ax3, r_test_after_opt[1,:],r_test_after_opt[2,:], color=(:blue,0.1), label=L"Pushed target samples through $S$")
	axislegend()
	fig3
end
...
begin
	...
	println("==================")
	println("Final coeffs component 1:")
	println(CoeffMap(S1))
	printfmt("Objective value for component 1: {:.2e}\n", obj(CoeffMap(S1), (S1, test_x)))
	println("==================")
	println("Final coeffs component 2:")
	println(CoeffMap(S2))
	printfmt("Objective value for component 2: {:.2e}\n", obj(CoeffMap(S2), (S2, test_x)))
	println("==================")
end
...
begin # Print statistics of normalized samples (TODO replace with better Gaussianity check)
	println("==================")
	mean_of_map = mean(r_test_after_opt, dims=2)
	println("Mean of normalized test samples")
	println(mean_of_map)
	println("==================")
	println("Cov of normalized test samples")
	cov_of_map = cov(r_test_after_opt, dims=2)
	println(cov_of_map)
	println("==================")
end
...
begin
	...
	ngrid=100
	x1_t = range(-3,3, length=ngrid)
	x2_t = range(-3,7.5,length=ngrid)
	xx1 = repeat(x1_t',ngrid,1)
	xx2 = repeat(x2_t ,1,ngrid)
...
begin
	fig4 = Figure()
	ax4 = Axis(fig4[1,1], title=L"S_1(x_1)", xlabel=L"x_1")
	lines!(ax4, x1_t, x1_t,label="true map", linewidth=3)
	lines!(ax4, x1_t, Evaluate(S1, collect(x1_t'))[:], linestyle=:dash, label="map approximation", linewidth=3)
	axislegend()
	fig4
end
...
fig5 = Figure()
ax5 = Axis(fig5[1,1], title=L"S_2(x_1,x_2)", xlabel=L"x_1", ylabel=L"x_2")
contour!(ax5, xx[1,:], xx[2,:], map_eval_true, linewidth=3)
contour!(ax5, xx[1,:], xx[2,:], vec(map_eval_approx), linestyle=:dash, linewidth=3)
fig5
end
...
function pullback_pdf(tri_map,rho,x)
    ...
    log_pdf = logpdf(rho, r)+LogDeterminant(tri_map, x)
    exp.(log_pdf)
end
...
function target_logpdf(x)
  rv = Normal()
  logpdf1 = logpdf(rv, x[1,:])
  logpdf2 = logpdf(rv, x[2,:]-x[1,:] .^2)
...
# ╔═╡ c6891f68-6ea4-4b65-85fe-fa5cfa3dfb5b
begin	
	...
	fig6 = Figure()
	ax6 = Axis(fig6[1,1], xlabel=L"x_1", ylabel=L"x_2")
	contour!(ax6, xx1[:], xx2[:], true_pdf_at_grid, label="Unnormalized target", linewidth=3)
	contour!(ax6, xx1[:], xx2[:], map_induced_pdf, linestyle=:dash, label="Map Approximation", linewidth=3)
	fig6
end
...
manifest_format = "2.0"
project_hash = "5dc9928a5275f633b48f328862f9500b29e5f6bd"
