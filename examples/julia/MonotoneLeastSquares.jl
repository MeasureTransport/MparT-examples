...
@info "Kokkos is using $(Concurrency()) threads"
...
begin
	...
	fig1 = Figure()
	ax1 = Axis(fig1[1,1], title="Reference data", xlabel=L"x", ylabel=L"H(x)")
...
begin
	...
	fig2 = Figure()
	ax2 = Axis(fig2[1,1], title="Training data", xlabel=L"x", ylabel=L"y")
...
begin
	...
	fig3 = Figure()
	ax3 = Axis(fig3[1,1], title="Starting map error: $error_before", xlabel=L"x", ylabel=L"y")
...
begin
	# Optimize
	u0 =  zeros(numCoeffs(monotone_map))
...
begin
	fig4 = Figure()
	ax4 = Axis(fig4[1,1], title="Final map error: $error_after", xlabel=L"x", ylabel=L"y")
	lines!(ax4, vec(x),vec(y_true),linestyle=:dashdot, label="true data", color=(:blue,0.8), linewidth=3)
	scatter!(ax4, vec(x),vec(y_measured),linestyle=:dashdot,label="noisy data", color=(:orange,0.4))
	lines!(ax4, vec(x)[1:end-1],vec(map_of_x_after)[1:end-1],linestyle=:dashdot,label="final map output", color=(:red,0.8), linewidth=3)
	axislegend()
	fig4
end

# ╔═╡ 6ceb1693-fbb5-4a5d-be41-0fdb3e44b074
md"""
