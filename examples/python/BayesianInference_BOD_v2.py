...
#
# Instead of computing the transport map approximation directly, we compute first a linear transformation to rescale the target density. Laplace approximation consists in finding a Gaussian approximation of the target density. This is done by finding the mode of the distribution which defines the mean of the Gaussian. The covariance matrix is then define with the square root of the inverse Hessian matrix of the log-pdf evaluated at the mode. The linear map is then define such that it transforms the Gaussian approximation to the standard normal.

# The objective function is define as the negative logpdf of the target density:

# +
def laplace_obj(x):
  return -log_target(x.reshape(-1,1)).flatten()

def grad_x_laplace_obj(x):
  return -grad_x_log_target(x.reshape(-1,1)).flatten()
...
res = scipy.optimize.minimize(laplace_obj, np.zeros((2,)), jac=grad_x_laplace_obj, method='BFGS', options=options)

# Mean is given by `res.x` and the inverse hessian approximation is give by `res.hess_inv`

# +
mean_laplace = res.x
cov_laplace = scipy.linalg.sqrtm(res.hess_inv)

# Cholesky factorization
A = np.linalg.cholesky(cov_laplace)

# Linear map
L = mt.AffineMap(np.asfortranarray(A),mean_laplace)


# -

# Visualization of the Laplace Approximation:
...
Zgauss = push_forward_pdf(L,ref,np.concatenate((X.reshape(1,-1),Y.reshape(1,-1)))).reshape(Ngrid,Ngrid)


fig, ax = plt.subplots()
CS2 = ax.contour(X, Y, Z)
CS3 = ax.contour(X, Y, Zgauss,linestyles='dashed')
...
# Transformed (pulled-back) target and its input gradient (should be integrated in next MParT release):

# +
def log_pullback(x):
    r = L.Evaluate(x)
    log_pdf = log_target(r)+L.LogDeterminant(x)
    return log_pdf

def grad_x_log_pullback(x):
  r = L.Evaluate(x)
  gxlpdf = grad_x_log_target(r)
  out = L.Gradient(x,gxlpdf)
  return out


# -

# Vizualisation of the new target density:

# +
x2 = np.linspace(-3, 3, Ngrid)
y2 = np.linspace(-3, 3, Ngrid)
X2, Y2 = np.meshgrid(x2, y2)

Xtest = np.concatenate((X2.reshape(1,-1),Y2.reshape(1,-1)))

Zpb = log_pullback(Xtest)
Zpb = np.exp(Zpb.reshape(Ngrid,Ngrid))

Ztar = log_target(Xtest)
Ztar = np.exp(Ztar.reshape(Ngrid,Ngrid))

ref = multivariate_normal(np.zeros(2),np.eye(2));


fig, ax = plt.subplots()
CS = ax.contour(X2, Y2, Zpb,linestyles='dashed')
CS2 = ax.contour(X2, Y2, Ztar)
ax.set_xlabel(r'$\theta_1$')
ax.set_ylabel(r'$\theta_2$')
h1,_ = CS.legend_elements()
h2,_ = CS2.legend_elements()
ax.legend([h1[0], h2[0]], ['Pullback target density', 'target density'])
plt.show()
# -

# The new target density is now centered on the origin and has a covariance matrix closer to identity
...
def obj(coeffs, tri_map,x):
    ...
    rho_of_map_of_x = log_pullback(map_of_x) #log_target has been replaced by log_pullback
...
def grad_obj(coeffs, tri_map, x):
    ...
    tri_map.SetCoeffs(coeffs)
    map_of_x = tri_map.Evaluate(x)
    sensi = grad_x_log_pullback(map_of_x) #grad_x_log_target has been replaced by grad_x_log_pullback
...
# ### Map Composition
#
# The map that represent the posterior distribution is define by the composition of the linear transformation $L$ and the optimized triangular map we just computed.

numMaps = 2
moveCoeffs = True
Tcomp = mt.ComposedMap([tri_map,L],moveCoeffs,numMaps)

# ## Accuracy checks
#
# ### Comparing density contours 
#
# Comparison between contours of posterior $\pi(\boldsymbol{\theta}|\mathbf{y})$ and contours of pushforward density $T_\sharp \eta$.

# +
xx_eval = np.vstack((X.flatten(),Y.flatten()))
Z2 = push_forward_pdf(Tcomp,ref,xx_eval)
...
var_diag = variance_diagnostic(Tcomp,ref,log_target,test_z)
